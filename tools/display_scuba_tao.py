...
psd_ref = nc.variables['power_spectrum_TAO'][:, :]
...
def plt_spectral_analysis(cix, ciy):
    ...
    ax1.plot(wavenumber[index, :], psd_ref[index, :], color='r', label='PSD TAO', lw=2)
    ...
    ax6.plot(wavenumber[index, :], psd_map[index, :]/psd_ref[index, :], color='k',
        label='Spectral ratio (PSD map / PSD TAO)', lw=2)
    ...
    ax6.set_ylabel("PSD map / PSD TAO", fontweight='bold')
    plt.legend(loc='best')
    ax7 = ax6.twiny()
...
fig, ax = plt.subplots()
bmap = Basemap(projection='cyl', llcrnrlat=-20, urcrnrlat=20, llcrnrlon=120, urcrnrlon=290, resolution='i')
...
xx, yy = bmap(lon, lat)
scatter = bmap.scatter(xx, yy, c=effective_resolution, s=40, lw=0)
cbar_scatter = bmap.colorbar(scatter, location='bottom', pad='30%')
cbar_scatter.set_label('Effective_resolution at mooring (days)', fontweight='bold')
